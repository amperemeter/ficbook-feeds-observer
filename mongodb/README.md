...
2. Создать папку проекта и разместить внутри нее файлы [index.js](index.js) и [package-mongodb.json](package-mongodb.json) из репозитория. Переименовать `index.js` и `package-mongodb.json` в `index.js` и `package.json` соответственно.
